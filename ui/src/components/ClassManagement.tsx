...
export function ClassManagement({ onBack, userRole, teacherUserId }: ClassManagementProps) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null);
      try {
        const [summaries, teacherList, cameraList, deptList] = await Promise.all([
        ...
        const mapped: Class[] = summaries.map((lecture) => {
          ...
          return {
            ...
            cameraId: lecture.camera?.camera_id ? String(lecture.camera.camera_id) : null,
        ...
        setAllDepartments(deptList.map((d: any) => d.name));
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <Card className="p-6">
            <div className="flex items-start justify-between">
              ...
              <div
                className={`w-12 h-12 ${stats.withoutTeachers > 0 ? "bg-red-100" : "bg-gray-100"
                  } rounded-lg flex items-center justify-center`}
              >
                <AlertTriangle
                  className={`w-6 h-6 ${stats.withoutTeachers > 0 ? "text-red-600" : "text-gray-400"
                    }`}
                />
              </div>
            </div>
      ...
      <EnrollmentForm
        ...
        onAddStudents={handleAddStudentToClass}
      />
    </div>
  );
}